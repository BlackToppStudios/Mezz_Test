...
namespace Testing
{
    ...
    ParsedCommandLineArgs::ParsedCommandLineArgs(const CoreTestGroup& TestInstances)
    {
        MEZZ_TRACE("Constructing command delegation table.")

        MEZZ_TRACE("Adding simple flags to delegation table.")
        CallingTable[HelpToken] = [this]() noexcept { ExitWithError = EXIT_FAILURE; };
        CallingTable[RunInThisProcessToken] = [this]() noexcept { InSubProcess = true; };
        CallingTable[NoThreads] = [this]() noexcept { ForceSingleThread = true; };
        CallingTable[JunitXMLAToken] = [this]() noexcept { EmitJunitXml = true; };
        CallingTable[JunitXMLBToken] = [this]() noexcept { EmitJunitXml = true; };
        CallingTable[SkipSummaryToken] = [this]() noexcept { SkipSummary = true; };
        CallingTable[SkipFileToken] = [this]() noexcept { SkipFile = true; };

        MEZZ_TRACE("Adding debug options to delegation table.")
        auto RunHere = [this]() noexcept
            { InSubProcess = true; ForceSingleThread = true; };
        ...
        for(const CoreTestGroup::value_type& OneTest : TestInstances)
        {
            ...
            CallingTable[OneTest.first] = [this, &OneTest]()
                { ScheduleTest(OneTest); };

            // Insert flag adjustments that force a test to be skipped.
            CallingTable[SkipTestToken+OneTest.first] = [this, &OneTest]()
                { SkipTest(OneTest); };
        }

        MEZZ_TRACE("Adding all test switch to delegation table.")
        CallingTable[AllToken] = [this, &TestInstances]()
        {
            DoAllTests = true;
        ...
        CallingTable[DefaultToken] = [this, &TestInstances]()
        {
            DoDefaultTests = true;
            for(CoreTestGroup::value_type OneTest : TestInstances)
            {
        ...
        CallingTable[AutomaticToken] = [this, &TestInstances]()
        {
            DoAutomaticTests = true;
            for(CoreTestGroup::value_type OneTest : TestInstances)
            {
        ...
        CallingTable[InteractiveToken] = [this, &TestInstances]()
        {
            DoInteracticeTests = true;
            for(CoreTestGroup::value_type OneTest : TestInstances)
            {
                if(!OneTest.second->ShouldRunAutomatically())
                    { ScheduleTest(OneTest); }
        ...
        CallingTable[DoBenchmarkToken] = [this, &TestInstances]()
        {
            DoBenchmarkTests = true;
            for(CoreTestGroup::value_type OneTest : TestInstances)
            {
    ...
    void ParsedCommandLineArgs::ScheduleTest(const CoreTestGroup::value_type& OneTest)
    {
    ...
    void ParsedCommandLineArgs::SkipTest(const CoreTestGroup::value_type& OneTest)
    {
    ...
    {
        ParsedCommandLineArgs Results(TestInstances);
        ...
        for (int c=1; c<argc; ++c)
        {
            ...
            if(Results.CallingTable.count(ThisArg))                               // check for keywords that aren't tests.
                { Results.CallingTable[ThisArg](); }
            else
            {
        ...
        if(Results.DoDefaultTests)
        {
            ...
            Results.CallingTable[DefaultToken]();
        }
    ...
    {
        MEZZ_TRACE("Rendering Tests results summary.")
    ...
    {
        MEZZ_TRACE("Rendering test timings.")
    ...
    {
        MEZZ_TRACE("Preparing test to execute test in...")
        if(Options.InSubProcess)
        {
            MEZZ_TRACE("    the same process.")
            OneTestGroup(); // Run tests and discard results, the parent process will grab it.
        } else {
            MEZZ_TRACE("    in an isolated process.")
    ...
    {
        MEZZ_TRACE("Scheduling tests in ...")
        ...
        for(const CoreTestGroup::value_type& OneTestGroup : TestInstances)
        {
            ...
            {
                MEZZ_TRACE("    to run in one thread.")
                DoAndTimeThisTest();
            }
            else
            {
                MEZZ_TRACE("    to run in a thread per test.")
