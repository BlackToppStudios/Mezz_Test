...
namespace Mezzanine
{
    namespace Testing
    {
        ...
        {
        public:
            /// @brief The type used to store the results of tests, largely used to clarify derived types.
        ...
        public:
            ...
            /// @brief Get the name of the test group for command line args and other user interaction.
            ...
            /// @brief Tells Test macros and AddResult whether or not to print immediately when called.
            ...
            /// @brief Is it safe to run this test class in parallel to other tests if done so in a process.
            ...
            /// @brief Decides if the test should run as part of a default invocation of the test suite.
            ...
            /// @brief An immutable iterator type suitable for use.
            typedef TestDataStorageType::const_iterator const_iterator;

            /// @return A mutable iterator to the beginning of the TestData.
            iterator begin();
            /// @return An immutable iterator to the beginning of the TestData.
            const_iterator begin() const;
            /// @return Be absolutely certain to get an immutable iterator to the beginning of the TestData.
            const_iterator cbegin() const;

            /// @return A mutable iterator to the end of the TestData.
            iterator end();
            /// @return An immutable iterator to the end of the TestData.
            const_iterator end() const;
            /// @return Be absolutely certian to get an immutable iterator to the end of the TestData.
            const_iterator cend() const;

            /// @brief If you have exactly the name you want in the report, use this to add the TestData.
            ...
            /// @brief The worst test result usually indicates the overall level of success. This returns that result.
            /// @return Scan every test and find the worst result among all test data.
            TestResult GetWorstResults() const;

            /// @brief Send everything in the test to the thread_local TestLog.
            ...
            /// @param FuncName The function that called the test, if blank.
            /// @param File To make tracking down failures easier you can pass in the file name of the test, if not set
            /// an empty string is used.
            /// @param Line To make tracking down failures easier you can pass in the line number of the test, if not
            /// set an empty string is used.
            ...
            /// @tparam ExpectedResultsType A type that supports comparison with the ActualResultsType on the left of
            /// the == operator and outputs to ostreams.
            /// @tparam ActualResultsType A type that supports comparison with the ExpectedResultsType on the right of
            /// the == operator and outputs to ostreams.
            ...
            /// @copydoc TestEqual
            /// @brief Test if an expected value and given value are equal or so close with floating point values as to
            /// not matter.
            /// @param EpsilonCount How many multiples of the epsilon should you allow the comparison to be off? A good
            /// rule of thumb is one per floating point operation that could introduce rounding, unless you have some
            /// other reason for tighter tolerances.
            /// @tparam ExpectedResultsType A type that supports comparison with the ActualResultsType on the left and
            /// right of the <= operator with ActualResultsType. ExpectedResultsType must output to ostreams via
            /// operator <<. This needs to have a specialization for std::numberic_limits<>::epsilon() implemented and
            /// that epsilon must be of the same type as or a similar enough type that it meets all these criteria as
            /// well. (At this point you should just read the template, it's not that complex and would be worth your
            /// time if your type is complex enough that its epsilon is a different type.) ExpectedResultsType results
            /// type must also support multiplication with a PreciseReal which is normally the largest and most precise
            /// primitive floating point type. So pretty much it needs to be a completely fleshed out or a completely
            /// primitive floating type.
            /// @tparam ActualResultsType A type that supports comparison with the ExpectedResultsType on the right and
            /// left <= operator with ExpectedResultsType. ActualResultsType outputs to ostreams.
            template <typename ExpectedResultsType, typename ActualResultsType>
            ...
            /// @param MaxVariance How many microseconds high or low are allowed in the test.
            ...
            /// you worked to optimize stayed optimized on new machines.
            /// @n @n
            /// Though less common this might make sense if trying to execute under some hard deadline.
            /// @param TestCallable A lambda or functor to call that should have predictable performance.
            /// @param MaxAcceptable The amount of microseconds the test cannot exceed.
            ...
            /// @param ExpectedNeedle This is the thing searched for in the ActualHaystack.
            /// @param ActualHaystack The string-like container that must contain the needle or the test fails.
